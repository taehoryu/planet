...
for i in profilefilelist:
  ...
  if time>=old_time+time_difference:
    ...
    dens=np.array(my_ray['density'][srt_ext])
    if j==0 :
      temp0=np.array(my_ray['Temp'][srt])
      ent0=np.array(my_ray['entropy'][srt])
      pressure0=np.array(my_ray['pressure'][srt])
      density0=np.array(my_ray['density'][srt_ext])

    diff_rho=abs(density0-dens)/density0
...
#  plt.ylim(10,10000)
    plt.xlim(1.e-9,2.e-2)

    plt.semilogy(density0,diff_rho,label='t='+filename_time+'s')
    plt.legend()
...
#  plt.arrow(0.2*10**8,8000,-0.1*10**8,2000,head_width=0.5e3, head_length=3e6)
  #plt.xaxis.set_major_formatter(FormatStrFormatter('10^{%T}'))#
    plt.xscale('log')
#  plt.yscale('linear')
    plt.savefig("x_rho_diff_1000.png")
